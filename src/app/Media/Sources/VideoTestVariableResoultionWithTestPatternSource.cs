...
{
    public class VideoTestVariableResoultionWithTestPatternSource : IVideoSource, IDisposable
    {
        //public const string TEST_PATTERN_RESOURCE_PATH = "SIPSorcery.media.testpattern.i420";
        public const string TEST_PATTERN_RESOURCE_PATHRegular = "SIPSorcery.media.640x480.bgr";
        public const string TEST_PATTERN_RESOURCE_PATHSquare = "SIPSorcery.media.500x500.bgr";
        public int TEST_PATTERN_WIDTH = 500;
        public int TEST_PATTERN_HEIGHT = 500;
        ...
        private byte[] _testBufferRegular;
        private byte[] _testBufferSquared;
        ...
        public VideoTestVariableResoultionWithTestPatternSource(IVideoEncoder encoder = null)
        {
            if (encoder != null)
            {
            ...
            var testPatternStm = assem.GetManifestResourceStream(TEST_PATTERN_RESOURCE_PATHRegular);

            if (testPatternStm == null)
            {
                OnVideoSourceError?.Invoke($"Test pattern embedded resource could not be found, {TEST_PATTERN_RESOURCE_PATHRegular}.");
            }
            else
            {
                SetupTestPattern();
        ...
        public void ToggleResolution()
        {
            ...
            if (showBig)
            {
                ...
                TEST_PATTERN_WIDTH = 640;
                TEST_PATTERN_HEIGHT = 480;
            }
            else
            {
                TEST_PATTERN_WIDTH = 500;
                TEST_PATTERN_HEIGHT = 500;
        ...
        bool showBig = false;
        private void SetupTestPattern()
        {

            var assem = typeof(VideoTestPatternSource).GetTypeInfo().Assembly;

            var testPatternStm = assem.GetManifestResourceStream(TEST_PATTERN_RESOURCE_PATHRegular);
            _testBufferRegular = new byte[640 * 480 * 3];
            testPatternStm.Read(_testBufferRegular, 0, _testBufferRegular.Length);
            testPatternStm.Close();

            _testBufferSquared = new byte[500 * 500 * 3];

            testPatternStm = assem.GetManifestResourceStream(TEST_PATTERN_RESOURCE_PATHSquare);
            testPatternStm.Read(_testBufferSquared, 0, _testBufferSquared.Length);
            testPatternStm.Close();
        ...
        private void GenerateTestPattern(object state)
        {
            ...
            if (showBig)
            {
                buffer = _testBufferRegular;
            }
            else
            {
                buffer = _testBufferSquared;
            }

            lock (_testBufferSquared) lock (_testBufferSquared)
                {
                    lock (_sendTestPatternTimer)
                    {
                        if (!_isClosed && (OnVideoSourceRawSample != null || OnVideoSourceEncodedSample != null))
                        {
                            ...
                            //StampI420Buffer(buffer, TEST_PATTERN_WIDTH, TEST_PATTERN_HEIGHT, _frameCount);

                            if (OnVideoSourceRawSample != null)
                            {
                                GenerateRawSample(TEST_PATTERN_WIDTH, TEST_PATTERN_HEIGHT, buffer);
                            }

                            if (_videoEncoder != null && OnVideoSourceEncodedSample != null && !_formatManager.SelectedFormat.IsEmpty())
                            {
                                var encodedBuffer = _videoEncoder.EncodeVideo(TEST_PATTERN_WIDTH, TEST_PATTERN_HEIGHT, buffer, VideoPixelFormatsEnum.Bgr, _formatManager.SelectedFormat.Codec);
